...
.. seealso::
   ...
   MegEngine 的 Data 包主体 API 设计与 PyTorch 类似，可参考 
   `TORCH.UTILS.DATA <https://pytorch.org/docs/stable/data.html>`_ 对比异同。

.. warning::

   * 相关操作均以 NumPy 的 :py:class:`~.numpy.ndarray` 作为处理数据，
     也不会产生 Tensor 类型的输出，因此如果后续步骤存在与 Tensor 有关的计算，
     则需要 :ref:`create-tensor-from-ndarray` 。
   * 处理 Data pipeline 不是只有使用 ``megengine.data`` 这一种途径，用户可以不借助 MegEngine, 
     使用其它框架或库来完成 :py:class:`~.numpy.ndarray` 数据的处理和加载，也可以自己实现所有的处理逻辑。

MegEngine 中的 :py:mod:`data` 子包提供了用于处理数据（数据集）的原语，
其中 :py:class:`megengine.data.DataLoader` 被用于加载数据，本质上将实例化成一个可迭代的对象，
负责从 :py:class:`~.Dataset` 的样例（包括数据样本和目标/标签）中随机返回批量大小 ``batch_size`` 的数据。
简而言之，``Dataset`` 对象将训练或测试数据加载到内存中，``DataLoader`` 对象从 ``Dataset`` 中获取数据并分批提供数据。
但在某些情况下，直接遍历 ``Dataset`` 中的数据会更方便。
...
.. note::
   ...
   #. 创建一个 Dataset 对象（如果有需要，可以自己定义 ``Sampler``, ``Transform`` 和 ``Collator`` ）；
   #. 创建一个 DataLoader 对象；
   #. 迭代这个 DataLoader 对象，将每批 ``data`` 和 ``label`` 加载到模型中进行训练；
   
   注：模型的验证和测试也可以是使用对应的 DataLoader 完成数据部分的加载。
...
下面我们以加载图像分类数据的基本流程作为例子（并非所有参数都进行了配置） ——

#. 假设图像数据按照一定的规则放置于同一目录下（通常数据集主页会对文件组织进行介绍）。
   ...
   要获取示例图像，可以调用该类并将一些随机索引传递给 ``__getitem__`` 方法，然后将返回图像数组和对应的标签，例如：
